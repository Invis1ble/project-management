<?php

declare(strict_types=1);

namespace Invis1ble\ProjectManagement\Shared\Infrastructure\Domain\EventLog\EventFormatter\DevelopmentCollaboration\MergeRequest;

use Invis1ble\Messenger\Event\EventInterface;
use Invis1ble\ProjectManagement\Shared\Domain\Event\DevelopmentCollaboration\MergeRequest\MergeRequestAwaitingTick;
use Invis1ble\ProjectManagement\Shared\Infrastructure\Domain\EventLog\EventFormatter\AbstractFormatter;

/**
 * @extends AbstractFormatter<MergeRequestAwaitingTick>
 */
final readonly class MergeRequestAwaitingTickFormatter extends AbstractFormatter
{
    public function supports(EventInterface $event): bool
    {
        return $event instanceof MergeRequestAwaitingTick;
    }

    public function format(EventInterface $event): string
    {
        return "MR $event->guiUrl awaiting tick (`$event->sourceBranchName` -> `$event->targetBranchName` | `$event->title`)";
    }
}
